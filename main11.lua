...
-- 1. 创建行为树
local bt = otherworld_bt.create(config, my_game_info)

-- 2. 调用节点注册检查函数
local function check_node_registration(tree_root, all_nodes)
    local undefined_nodes = {}
    local function _check_node(node)
        if not all_nodes[node.name] then
            table.insert(undefined_nodes, {
                id = node.id,
                name = node.name,
                type = node.type
            })
    ...
    _check_node(tree_root)
    if #undefined_nodes > 0 then
        print("\n=== 未注册的节点 ===")
        for _, node in ipairs(undefined_nodes) do
            print(string.format("ID: %s | 名称: %s | 类型: %s", node.id, node.name, node.type))
        end
        error("存在未注册的节点！")
    else
        print("所有节点均已注册到 all_nodes。")
    end
end

-- 执行检查（传入行为树根节点和合并后的节点表）
check_node_registration(bt.tree.root, all_nodes)

-- 3. 继续运行行为树
for i = 1, 10 do
    print("\n=== Tick " .. i .. " ===")
    otherworld_bt.run(bt)
end