...
local custom_nodes = {
            ...
            if poe2_api.click_text_UI({text = "life_orb",UI_info = env.UI_info})
             or poe2_api.click_text_UI({text = "resume_game",UI_info = env.UI_info})
              or poe2_api.find_text({text = "清單",UI_info = env.UI_info,min_x = 0,min_y = 0,max_x = 400}) then
                ...
                if not self.bool and not poe2_api.table_contains(poe2_api.get_team_info(env.player_info , 2),{"大號名","未知"}) then
    ...
    RestController = {
        run = function(self, env)
            ...
            local start_time = api_GetTickCount64() -- 开始时间
            -- 特殊情况跳出
            if poe2_api.get_team_info(env.player_info , 2) ~="大號名" or player_info.isInBossBattle or poe2_api.is_have_mos({range_info = env.range_info , player_info = env.player_info , dis = 80 , stuck_monsters = env.stuck_monsters, not_attack_mos = env.not_attack_mos }) then
                return bret.SUCCESS
            end
                -- 初始化检查
    ...
    Check_LongTime_EXP_Add = {
        run = function(self, env)
            ...
            local player_info = env.player_info
            -- 特殊情况跳出
            if poe2_api.get_team_info(player_info , 2) ~="大號名" then
                return bret.SUCCESS
            end
            -- 检查是否处于停滞移动状态
    ...
    Is_Deth = {
        run = function(self, env)
            ...
            if player_info.life == 0 and poe2_api.find_text({UI_info = env.UI_info, text="在記錄點重生"}) then
                ...
                if (start_time - self.death_time >= 60 * 1000) and (poe2_api.get_team_info( player_info, 2) == "大號名") then
    ...
    Not_Main_Page = {
        run = function(self, env)
            ...
            if count_gravestones(current_map_info) == 0 then
                ...
                self.life_time = 0
                env.is_timeout = false
            elseif count_gravestones(current_map_info) > 0 and poe2_api.get_team_info(player_info, 2) ~= "大號名" and player_info.life >0 and player_info.isInBossBattle and not poe2_api.is_have_mos({range_info = env.range_info, player_info = player_info}) then
            ...
            if not string.find(player_info.current_map_name_utf8, "own") then
                -- 超时记录点重生
                if env.is_timeout and poe2_api.get_team_info(player_info, 2) == "大號名" then
