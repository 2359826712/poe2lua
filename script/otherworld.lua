...
package.loaded['script\\my_game_info'] = nil
...
local account_path = script_dir .. "\\Account.txt"
...
local plot_nodes = {
    ...
    Get_User_Config_Info = {
        run = function(self, env)
            ...
            if not env.user_config then
                ...
                -- 查找最小攻击距离
                local skill_config = {}
                if skills_config then
                    skill_config = skills_config
                else
                    skill_config = config["技能設置"] or {}
                end
    ...
    Join_Game = {
        run = function(self, env)
            ...
            if env.speel_ip_number >= 50
                ...
                or env.is_ban
                or env.add_friend_max
                or env.to_level  then
                ...
                poe2_api.dbgp("is_ban:", env.is_ban)
                poe2_api.dbgp("add_friend_max:", env.add_friend_max)
                poe2_api.dbgp("to_level:", env.to_level)
                ...
                if pid and next(pid) and pid[1] ~= 0 then
                    ...
                    -- api_SetWindowState(env.game_window, 13)
                    if not poe2_api.find_text({UI_info = env.UI_info, text = game_str.Leave_the_game,min_x = 750 ,max_x = 840,max_y = 550}) then
                        poe2_api.click_keyboard("esc")
                        api_Sleep(1000)
                        return bret.RUNNING 
                    end
                    poe2_api.find_text({UI_info = env.UI_info, text = game_str.Leave_the_game,min_x = 750 ,max_x = 840,max_y = 550,click = 2})
                    api_Sleep(2000)
            ...
            if poe2_api.find_text({ text = "建立帳號", UI_info = env.UI_info, min_x = 0, max_y = 790 })
                ...
                api_Sleep(500)
                local text = poe2_api.get_name_text()
            ...
            if env.player_info.level >= 2 and env.shouting_method == "0" then
                ...
                local function ChatWithAI(content, api_key, opts)
                    ...
                    local system_prompt = table.concat({
                        ...
                        "Core message: Let players know it is a website related to in-game items; it is formed from SKILLGEM and 'gg' with a dot, but do not write them together or show the full URL.",
                        "Vary the phrasing; do not repeat these words verbatim.",
                        "No emojis.",
                ...
                local function GenerateSalesContent()
                    ...
                    local content = "Write ONE English in-game chat sentence that lets players know it is a website related to in-game items, formed from SKILLGEM and 'gg' separated by a dot; do not write them adjacent or show the full URL; vary the phrasing."
    ...
    Level_2 = {
        run = function(self, env)
            if env.level >= 2 and env.shouting_method == "7" then 
                poe2_api.print_log("2级")
                local local_ip = poe2_api.get_local_ip()
                local names = nil
                if local_ip then
                    local status_code1, response1 = poe2_api.query_data_game({game_name = "steam_account", status = 0, computer_number = local_ip})
                    if status_code1 == 200 and type(response1) == "table" then
                        ...
                        poe2_api.update_data_game({game_name = "steam_account",account=names[1],level = 2})
                    else
                        poe2_api.dbgp("查询数据失败")
                        env.to_level = true
                    end
                end
            end
            ...
            local function is_monster(range_info,mate,distance)
                ...
                for _, v in ipairs(range_info) do
                    if v.type == 1 and not v.is_friendly and v.life > 0
                     ...
                     and (not stuck_monsters or not next(stuck_monsters) or not poe2_api.table_contains(v.id,stuck_monsters)) then
                        local dis = poe2_api.point_distance(v.grid_x, v.grid_y,mate)
                        if dis and dis < distance then
                            return true
                        end
        ...
        run = function(self, env)
            ...
            if team_info_data and #team_info_data == num then
                ...
                for _, member in ipairs(team_info_data) do
                    if member.roleStatus == 2 or (member.roleStatus == 0 and member.name_utf8 == leader_name) then
                        a = a + 1
                    end
                end
                if a == num then
                    ...
                    for _, member in ipairs(team_info_data) do
                        if not poe2_api.table_contains(member.name_utf8,{ leader_name}) and not poe2_api.table_contains(member.name_utf8,env.team_info["小號名"])then
            ...
            elseif player_info.name_utf8 ~= leader_name then
                ...
                for _, member in ipairs(team_info_data) do
                    if member.roleStatus == 0 and member.name_utf8 == leader_name then
                        poe2_api.dbgp("是指定队长")
                        ...
                        poe2_api.time_p("Team", api_GetTickCount64() - current_time)
                        return bret.RUNNING
                    elseif member.roleStatus == 0 and member.name_utf8 ~= leader_name then
            ...
            if player_info.name_utf8 == leader_name then
                ...
                if not env.redis_initialized then
                    -- 订阅以大号名称为名的频道
                    local channel_name = leader_name.."_team"
            ...
            elseif player_info.name_utf8 ~= leader_name then
                ...
                if (not env.bool1) or (api_GetTickCount64() - self.current_time > 1000 * 2) then
                    ...
                    local success = api_RedisSendText(channel_name, send_data)
            ...
            if team_info_data and #team_info_data >= num and leader_name == player_info.name_utf8 then
                for _, member in ipairs(team_info_data) do
                    -- 检查成员是否不在队伍信息中
                    local found = false
    ...
    Query_Current_Task_Information = {
        run = function(self, env)
            ...
            if poe2_api.find_text({ UI_info = env.UI_info, text = "不在線上", min_x = 0, min_y = 0, max_x = 195, max_y = 590 })  and not check_pos_dis(team_member_3) then
                env.raw = {}
                poe2_api.dbgp("大号不在線上")
                return bret.RUNNING
            end
            poe2_api.printTable(api_GetQuestList(0))
            ...
            if task and next(task) then
                ...
                if poe2_api.table_contains(env.task_name , {"找到塞爾卡里庇護所","在卡里交匯道尋找賈萊關口"}) and poe2_api.table_contains(player_info.current_map_name_utf8 , {"P2_1","P2_Town"}) then
                    task = poe2_api.get_task_info(main_task.tasks_data,env.task_name)
                else
                ...
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G2_town"}) and task.task_name == "返回車隊，與芮蘇討論封閉的古老關口" then
                    ...
                    env.map_name = "G2_town"
                    env.interaction_object = { "芮蘇" }
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, {  "G2_9_2" }) and task.task_name == "擊敗憎惡者．賈嫚拉" then
    ...
    Is_Pick_Up_Task_Props = {
        run = function(self, env)
            ...
            local user_config = env.user_config
            local current_time = api_GetTickCount64()
            local user_new_item = poe2_api.get_BD_info("女巫", "装备信息")
    ...
    Need_Twice_Teleport = {
        run = function(self, env)
            ...
            if string.find(player_info.current_map_name_utf8, "own") and special_map_point and not poe2_api.table_contains(my_profession, { "大号名称", "未知" }) then
                ...
                if special_map_point and player_info.current_map_name_utf8 == "G2_town" and poe2_api.find_text({ UI_info = env.UI_info, text = "哈拉妮關口", min_x = 195, refresh = true }) then
                    if poe2_api.find_text({ UI_info = env.UI_info, text = "快行" }) then
                        poe2_api.click_keyboard("space")
                    end
    ...
    Is_Teleport = {
        run = function(self, env)
            ...
            local team_member_2 = poe2_api.get_team_info(team_info, user_config, player_info, 2)
            local team_member_3 = poe2_api.get_team_info(team_info, user_config, player_info, 3)
            local team_member_5 = poe2_api.get_team_info(team_info, user_config, player_info, 5)
            local boss_name = env.boss_name
            local interaction_object = env.interaction_object
            local task_name = env.task_name
            local special_map_point = env.special_map_point
            ...
            if not self.follow and ((special_map_point and interaction_object and string.find(me_area, "G2_town")) or 
                ...
                (poe2_api.table_contains(party_pos(team_member_5[1]), { "G2_1","G4_2_2" }) and not poe2_api.table_contains(interaction_object, "阿杜拉車隊") )) then                
                ...
                if not check_pos_dis(team_member_5[1]) and poe2_api.find_text({ UI_info = UI_info, text = waypoint_name_utf8, min_x = 0, min_y = 0, max_x = 195, max_y = 590 }) then
                    for i = 1, count do
                        if not poe2_api.find_text({ UI_info = UI_info, text = "你確定要傳送至此玩家的位置？" }) then
                            local x, y = poe2_api.get_member_name_according(UI_info, team_member_5[1])
                            poe2_api.dbgp("x,y",x,y)
                            if y ~= 0 then
                                local rand_x = 14 + math.random(-7, 7)
                                local rand_y = y + 21 + math.random(-7, 7)
            ...
            if poe2_api.table_contains(me_area, { "G3_12" }) and poe2_api.table_contains(task_area, { "G3_12" }) and not poe2_api.is_have_boss_distance(range_info, player_info, boss_name) then
                ...
                if next_level() and check_pos_dis(team_member_5[1]) and check_pos_dis(team_member_5[1]) > 30 and not check_role_path(team_member_5[1]) then
                    ...
                    return bret.RUNNING
                elseif not check_pos_dis(team_member_5[1]) then
                    return bret.RUNNING
                else
                    env.louti_space = true
                    self.follow = false
            ...
            if me_area == task_area then
                ...
                if poe2_api.table_contains(me_area, { "G1_15", "G2_3", "G3_12"}) then
                    ...
                    if not check_pos_dis(team_member_5[1]) and team_member_2 ~= "未知" then
                        return bret.RUNNING
                    end
        ...
        run = function(self, env)
            ...
            if player_info.current_map_name_utf8 == "G1_1" then
                ...
                local function is_adjust_skills()
                    ...
                    local skills = poe2_api.get_BD_info("女巫", "技能")
                ...
                local function is_exist(name)
                    ...
                    local level = poe2_api.get_BD_info("女巫", "技能", env.skill_name, "level_skillstone")
                ...
                if  bool then
                    ...
                    local skill = poe2_api.get_BD_info("女巫", "技能", env.skill_name, "skill_name")
    ...
    Is_Skill_In_Skill_List = {
        run = function(self, env)
            ...
            -- 检查技能是否存在
            local skill_config_name = poe2_api.get_BD_info("女巫", "技能", skill_name, "skill_name")
    ...
    ReleaseSkillAction = {
        run = function(self, env)
            ...
            local function _select_skill(available_skills)
                ...
                local skill_setting = env.skills_config
            ...
            if valid_monsters then
                ...
                poe2_api.dbgp("ReleaseSkillAction bret.RUNNING")
                return bret.RUNNING
            end
            ...
            local _handle_space_action_path_name = function(player_info)
                ...
                if danger and danger.inside then
                    ...
                    if safe_point and safe_point.x ~= -1 and safe_point.y ~= -1 then
                        ...
                        if env.player_info.current_map_name_utf8 == "G1_1" then
                            poe2_api.click_keyboard("space")
                            api_Sleep(200)
                        end
    ...
    Is_Have_Interaction_Object = {
        run = function(self, env)
            ...
            if me_area == "G1_1" then
                ...
                if poe2_api.find_text({ UI_info = env.UI_info, text = "大箱子",min_x = 0,max_x = 1420}) then
                    env.end_point = nil
                    env.is_arrive_end = false
                    env.path_list = {}
                    if self.last_click_time == 0 then
    ...
    The_Interactive_Object_Exist = {
        run = function(self, env)
            ...
            if interaction_object or interaction_object_map_name then
                ...
                if player_info.current_map_name_utf8 == "P1_6" and team_member_2 =="大号名称" then
                    local boss = poe2_api.get_sorted_obj("艾兒薇絲女士", range_info, player_info)
                    if boss and #boss > 0 and boss[1].life > 0 then
                        env.record_map = nil
                        if get_distance(boss[1].grid_x,boss[1].grid_y) < 35 then
                            poe2_api.dbgp("boss距离<35")
                            return bret.RUNNING
    ...
    Interactive = {
        run = function(self, env)
            ...
            if team_member_2 == "大号名称" and poe2_api.table_contains("馬提奇", interaction_object) then
                env.is_not_ui = true
                return bret.RUNNING
            end
        ...
        run = function(self, env)
            ...
            local function need_move(obj,dis)
                ...
                local ok, value = pcall(function() 
                    return obj.baseType_utf8 
                end)
                if ok and value ~= nil then
                    text = obj.baseType_utf8 
                    -- text = ""
                else
                    text = obj.name_utf8
                end
                            ...
                            if env.need_item and env.need_item == interactive_object then
                                ...
                                if ok and value ~= nil then
                                    ...
                                    elseif game_str.Monolith_PATH == interactive_object.path_name_utf8 then
                                        if not poe2_api.find_text({text = game_str.Monsters_are_imprisoned_by_powerful_essence_TWCH, UI_info = env.UI_info, min_x=200,max_y=750,match=2,max_x=1200,sorted = true, click=2}) then
                                            api_ClickMove(poe2_api.toInt(interactive_object.grid_x), poe2_api.toInt(interactive_object.grid_y), 1)
                                        end
    ...
    Add_Friend = {
        run = function(self, env)
            ...
            if poe2_api.find_text({ UI_info = env.UI_info, text = game_str.Refuse, min_x = 0, max_x = 600, click = 2 }) then
                poe2_api.dbgp("执行拒绝操作")
                api_Sleep(2000)
                return bret.RUNNING
            end
            poe2_api.dbgp("self.click_Friend_number:",self.click_Friend_number)
            ...
            --                     poe2_api.find_text({UI_info = env.UI_info, text = "HunterManAR",min_x = 140,  man_x = 180, match = 2,click = 7})
            --                     api_Sleep(1500)
            --                     poe2_api.find_text({UI_info = env.UI_info, text = "HunterManAR",min_x = 140,  man_x = 180, match = 2,click = 2,add_x = 150,add_y = 200})
            --                     api_Sleep(1000)
            --                     return bret.RUNNING
            --                 else
            ...
            --                 if not poe2_api.find_text({UI_info = env.UI_info, text = "你確定要從好友名單中將", man_x = 180, match = 2}) then
            --                     poe2_api.dbgp("点击删除添加好友")
            --                     poe2_api.find_text({UI_info = env.UI_info, text = "HunterManAR",min_x = 0,  man_x = 180, match = 2,click = 7})
            --                     api_Sleep(1500)
            --                     poe2_api.find_text({UI_info = env.UI_info, text = "HunterManAR",min_x = 0,  man_x = 180, match = 2,click = 2,add_x = 150,add_y = 215})
            --                     api_Sleep(1000)
            --                     return bret.RUNNING
            --                 else
            --                     -- poe2_api.find_text({UI_info = env.UI_info, text = "是", min_x = 930,click = 2})
            ...
            --             end
            --         else
            --             env.click_Friend_zdy = false
            --             self.click_Friend_number = 0
            ...
            --         api_Sleep(1000)
            --         poe2_api.paste_text1("HunterManAR")
            --         api_Sleep(1000)
            --         poe2_api.click_keyboard("enter")
            --         api_Sleep(4000)
            --         env.click_Friend_zdy = true
            --         -- table.remove(env.Id_list,1)
            --         return bret.RUNNING
            --     end
            -- end
            if not env.text_ui_list then
                local a = poe2_api.click_text_UI({UI_info = env.UI_info, text = "#", min_x = 20 , max_x = 30,index = 1})
            ...
            if env.text_ui_list and next(env.text_ui_list) then
                poe2_api.dbgp("删除好友")
                poe2_api.dbgp(env.text_ui_list[1])
                api_RemoveFriend(env.text_ui_list[1])
                table.remove(env.text_ui_list,1)
                api_Sleep(200)
                -- table.remove(env.Id_list,1)
                return bret.RUNNING
                -- if not poe2_api.find_text({UI_info = env.UI_info, text = "你確定要從好友名單中將", man_x = 180, match = 2}) then
                ...
                --     return bret.RUNNING
                -- end
            elseif env.text_ui_list then
                env.text_ui_list = nil
            end
            ...
            if env.Id_list and next(env.Id_list) then
                ...
                if not self.click_Friend_number then
                    self.click_Friend_number = 0
                end
                for i,v in ipairs(env.Id_list) do
                    api_AddFriend(env.Id_list[i])
                    api_Sleep(300)
                    self.click_Friend_number = self.click_Friend_number + 1
                end
                env.Id_list = {}
                ...
                local status_code, response = poe2_api.query_data("poe2", 4320, 1, 10)
                if status_code == 200 then
                    local data = response and response.data
                    local names = {}
                    if type(data) == "table" then
