...
-- end
-- 根据ini转成lua
_M.check_Setting_config = function(config_path)
    local file = io.open(config_path, "r") -- 打开文件
...
_M.get_account_info=function(account_path)
    ...
    if #current_account ~= 6 then
        _M.dbgp("请确保账号信息配置格式：账号|密码|服务器|等级|角色|流派|")
        return nil
    end
