...
local custom_nodes = {
            ...
            if player_info.life ~= 0 and not poe2_api.click_text_UI({ text = "respawn_at_checkpoint_button", UI_info = env.UI_info }) then
                if (string.match(player_info.current_map_name_utf8, "town") and not self.bool) or start_time - self.time > 5 * 60 * 1000 then
                    if not poe2_api.find_text({ UI_info = env.UI_info, text = "/clear", min_x = 0 }) then
                        api_ClickMove(poe2_api.toInt(player_info.grid_x), poe2_api.toInt(player_info.grid_y),
                            poe2_api.toInt(player_info.world_z), 7)
                        api_Sleep(1000)
                        poe2_api.click_keyboard("enter", 0)
                        api_Sleep(500)
    ...
    RestController = {
        run = function(self, env)
            ...
            if not self._is_initialized then
                ...
                self.work_duration_ms = math.floor(base_work * 60 * 1000 *
                    (1 + (math.random() * work_random_range * 2 - work_random_range)))
                ...
                self.rest_duration_ms = math.floor(base_rest * 60 * 1000 *
                    (1 + (math.random() * rest_random_range * 2 - rest_random_range)))
                ...
                poe2_api.dbgp("初始化完成 - 工作时间:%d分钟 休息时间:%d分钟", self.work_duration_ms / (60 * 1000),
                    self.rest_duration_ms / (60 * 1000))
                return bret.RUNNING
            end
    ...
    Check_LongTime_Not_Move = {
        run = function(self, env)
            poe2_api.print_log("开始执行长时间未移动检查...")
            local current_time = api_GetTickCount64()
            ...
            if is_moving and real_stagnation_time_move > space_time * 1000 then
                ...
                if not take_rest then
                    ...
                    if env.range_info and player_info then
                        ...
                        if target then
                            api_ClickMove(poe2_api.toInt(target.grid_x), poe2_api.toInt(target.grid_y),
                                poe2_api.toInt(player_info.world_z), 1)
                            api_Sleep(200)
    ...
    Story_Discard = {
        run = function(self, env)
            ...
            if not Attachments or not next(Attachments) then
                ...
                if point then
                    if poe2_api.find_text({text = "背包",UI_info = env.UI_info,min_x=1000,min_y=32,max_x=1600,max_y=81}) then
                        api_ClickScreen(poe2_api.toInt(point[1]),poe2_api.toInt(point[2]),1)
                        api_Sleep(100)
    ...
    Is_Pick_UP = {
        run = function(self, env)
                            ...
                            if poe2_api.match_item(item,cfg,1) then
                                ...
                                if not cfg['不撿'] then
                                    ...
                                    if distance and distance > 15 then
                                        ...
                                        if not ralet and not next(ralet) then
                                            poe2_api.dbgp("无路径")
                                            break
                                        end
                                    end
            ...
            if poe2_api.find_text({text = "背包" ,UI_info = env.UI_info,min_x=0,min_y=32,max_x=1600,max_y=81}) then
                poe2_api.click_keyboard("i")
                poe2_api.dbgp("关背包7")
                api_Sleep(200)
    ...
    Open_Warehouse = {
        run = function(self, env)
            ...
            if distance > 25 then
                ...
                api_ClickMove(poe2_api.toInt(warehouse.grid_x), poe2_api.toInt(warehouse.grid_y),
                    poe2_api.toInt(player_info.world_z), 1)
    ...
    Query_Current_Task_Information_Local = {
        run = function(self, env)
            ...
            local function party_member_map(map)
                ...
                for _, member in ipairs(team_info_data) do
                    if string.find(map, member["current_map_name_utf8"]) then
                        return true
                    end
                end
                return false
            ...
            if string.find(player_info.current_map_name_utf8, "C_") then
                twice_map = false
            else
                twice_map = true
            end

            if string.find(party_pos(poe2_api.get_team_info(team_info, user_config, player_info, 4)), "C_") or party_pos(poe2_api.get_team_info(team_info, user_config, player_info, 4)) == "G_Endgame_Town" then
                poe2_api.dbgp("红色章节")
            ...
            if next(task) then
                ...
                if party_member_map({ "G3_6_2", "C_G3_6_2" }) and task.task_name == "與艾瓦對話" then
                    poe2_api.dbgp("[Query_Current_Task_Information_Local]與艾瓦對話")
                    if twice_map then
                        env.map_name = "G3_6_2"
                    else
                        env.map_name = "C_G3_6_2"
                    end
                    env.interaction_object_map_name = { "艾瓦" }
                    env.interaction_object = { "艾瓦" }
                elseif party_member_map({ "G3_6_2", "C_G3_6_2" }) and task.task_name == "召喚艾瓦，尋求她的意見" then
                ...
                elseif party_member_map({ "G2_town", "C_G2_town" }) and task.task_name == "返回車隊，與芮蘇討論封閉的古老關口" then
                    poe2_api.dbgp("[Query_Current_Task_Information_Local]返回車隊，與芮蘇討論封閉的古老關口")
                    if twice_map then
                        env.map_name = "G2_town"
                    else
                        env.map_name = "C_G2_town"
                    end
                    env.interaction_object = { "芮蘇" }
                elseif party_member_map({ "C_G2_9_2_", "G2_9_2" }) and task.boss_name and poe2_api.table_contains(task.boss_name, "憎惡者．賈嫚拉") then
                    poe2_api.dbgp("[Query_Current_Task_Information_Local]憎惡者．賈嫚拉")
                    if twice_map then
                        env.map_name = "G2_9_2"
                    else
                        env.map_name = "C_G2_9_2_"
                    end
            ...
            if not next(task) then
                ...
                if maps then
                    ...
                    if maps == "C_G1_1" then
                        poe2_api.dbgp("检测到C_G1_1：")
                        env.interaction_object = { "皆伐營地" }
                        env.boss_name = { "浮腫米勒" }
                        env.grid_x = nil
                        env.grid_y = nil
                        env.interaction_object_map_name = { "MillerActive", "C_G1_town" }
                    elseif maps == "G3_7" then
                    ...
                    elseif maps == "C_G3_7" then
                        poe2_api.dbgp("检测到C_G3_7：")
                        env.interaction_object = { "召喚瑟維", "瑟維" }
                    end
                    return bret.SUCCESS
                end
                if string.find(player_info.current_map_name_utf8, "C_") or (maps and string.find(maps, "C_")) then
                    poe2_api.dbgp("红色章节")
                    env.tasks_data = main_task.tasks_data_hard
                else
                    poe2_api.dbgp("白色章节")
        ...
        run = function(self, env)
            ...
            local function get_ordered_quest_status(api_result, main_task_order)
                ...
                for _, task in ipairs(api_result) do
                    ...
                    table.insert(task_dict[main_quest], { sub_quest, dec_quest }) -- 存储状态和详情
                end

                local quest_details = {}
                for _, main_task in ipairs(main_task_order) do
                    if task_dict[main_task] then
                        -- 过滤掉'任務完成'的子任务，保留未完成的状态和详情
                        local unfinished_quests = {}
                        for _, quest in ipairs(task_dict[main_task]) do
                            if quest[1] ~= '任務完成' then
                                table.insert(unfinished_quests, quest)
                            end
                        end

                        -- 如果有未完成的子任务，则添加第一个的详情
                        if #unfinished_quests > 0 then
                            local detail = unfinished_quests[1][2]
                            local is_duplicate = false
                            for _, existing_detail in ipairs(quest_details) do
                                if existing_detail == detail then
                                    is_duplicate = true
                                    break
                                end
                            end
            ...
            local main_task_info = get_ordered_quest_status(api_GetQuestList(index), my_game_info.mian_task)
            if next(main_task_info) then
                if (poe2_api.table_contains(main_task_info, "追尋傳奇人物奧爾巴拉的腳步，重鑄瓦斯提里的戰角") or poe2_api.table_contains(main_task_info, "跟艾瓦談談發生的事")) and #(main_task_info) > 1 then
                    task = poe2_api.get_task_info(main_task_info[2])
                else
                    task = poe2_api.get_task_info(main_task_info[1])
                end
            else
                task = nil
            end
            if next(task) then
                ...
                if not poe2_api.table_contains(player_info.current_map_name_utf8, { "G3_town", "C_G3_town" }) and task.task_name == "回到過去，進入奧札爾" then
                    poe2_api.dbgp("[Query_Current_Task_Information]回到過去，進入奧札爾")
                    if twice_map then
                        env.map_name = "G3_town"
                    else
                        env.map_name = "C_G3_town"
                    end
                    task.task_name = "高地神塔營地"
                    env.task_name = task.task_name
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G3_6_2", "C_G3_6_2" }) and task.task_name == "與艾瓦對話" then
                    ...
                    env.interaction_object_map_name = { "艾瓦" }
                    env.interaction_object = { "艾瓦" }
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G3_6_2", "C_G3_6_2" }) and task.task_name == "召喚艾瓦，尋求她的意見" then
                    ...
                    env.interaction_object = { '中型靈魂核心', ' <questitem>{發電機}', '門' }
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G2_town", "C_G2_town" }) and task.task_name == "返回車隊，與芮蘇討論封閉的古老關口" then
                    ...
                    env.interaction_object = { "芮蘇" }
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "C_G2_9_2_", "G2_9_2" }) then
                ...
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G2_4_1" }) and not poe2_api.check_item_in_inventory("卡巴拉部落聖物", bag_info) then
                    ...
                    env.interaction_object_map_name = nil
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "C_G2_4_1" }) and (bag_object_sum("卡巴拉部落聖物") ~= 2) then
                    poe2_api.dbgp("[Query_Current_Task_Information]收集卡巴拉部落聖物 - 2")
                    task.task_name = "凱斯城"
                    env.task_name = task.task_name
                    env.map_name = "C_G2_4_1"
                    env.boss_name = { "異界．干擾女王．卡巴拉" }
                    env.interaction_object = { "卡巴拉部落聖物" }
                    env.interaction_object_map_name = nil
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G2_5_2" }) and not poe2_api.check_item_in_inventory("太陽部落聖物", bag_info) then
                    ...
                    env.interaction_object_map_name = nil
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "C_G2_5_2" }) and (bag_object_sum("太陽部落聖物") ~= 2) then
                    poe2_api.dbgp("[Query_Current_Task_Information]收集太陽部落聖物 - 2")
                    task.task_name = "骨坑"
                    env.task_name = task.task_name
                    env.map_name = "C_G2_5_2"
                    env.interaction_object = { "太陽部落聖物" }
                    env.interaction_object_map_name = nil
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G3_14" }) and not poe2_api.check_item_in_inventory("犧牲之心", bag_info) then
                    ...
                    env.interaction_object_map_name = nil
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "C_G3_14" }) and (bag_object_sum("犧牲之心") ~= 2) then
                    poe2_api.dbgp("[Query_Current_Task_Information]收集犧牲之心 - 2")
                    task.task_name = "奧札爾"
                    env.task_name = task.task_name
                    env.map_name = "C_G3_14"
                    env.interaction_object = { "犧牲之心" }
                    env.interaction_object_map_name = nil
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "G3_7" }) and not poe2_api.check_item_in_inventory("傑洛特顱骨", bag_info) then
                    ...
                    env.interaction_object_map_name = nil
                elseif poe2_api.table_contains(player_info.current_map_name_utf8, { "C_G3_7" }) and not poe2_api.check_item_in_inventory("寶石殼顱骨", bag_info) then
                    poe2_api.dbgp("[Query_Current_Task_Information]收集寶石殼顱骨")
                    task.task_name = "阿札克泥沼"
                    env.task_name = task.task_name
                    env.map_name = "C_G3_7"
                ...
                if not poe2_api.table_contains(player_info.current_map_name_utf8, { "G1_1" }) or not self.mas then
                    if not env.hard_chapter then
                        self.mas, finished_tasks = poe2_api.check_task_map_without(0)
                    else
                        self.mas, finished_tasks = poe2_api.check_task_map_without(1)
                    end
                    if not self.mas and not next(finished_tasks) then
                        poe2_api.print_log("没有任务信息")
                        return bret.RUNNING
                    end
                    if poe2_api.table_contains(finished_tasks, "部落復仇") or (self.mas and string.find(self.mas, "C_")) then
                        env.hard_chapter = true
                        env.tasks_data = main_task.tasks_data_hard
                    else
                        env.hard_chapter = false
                        env.tasks_data = main_task.tasks_data
                    end
                else
                    if self.mas and string.find(self.mas, "C_") then
                        env.hard_chapter = true
                    end
                end
                if player_info.level < 2 or player_info.current_map_name_utf8 == "G1_1" then
                    ...
                    if not env.hard_chapter then
                        if self.mas == "G2_town" then
                            ...
                            waypoint = api_GetTeleportationPoint()
                            if poe2_api.Waypoint_is_open("G3_town", waypoint) then
                                return bret.RUNNING
                            end
                            if player_info.current_map_name_utf8 ~= "G2_1" then
                                self.mas = "G2_1"
                            end
                        ...
                        if player_info.current_map_name_utf8 == "C_G1_1" then
                            env.interaction_object = { "皆伐營地" }
                            if poe2_api.get_team_info(team_info, config, player_info, 2) ~= "大號名" then
                                if not next(self.raw) or (self.raw_time ~= 0 and api_GetTickCount64() - self.raw_time > 16 * 1000) then
                                    -- 发送任务信息
                        ...
                        elseif self.mas == "C_G2_town" then
                            poe2_api.dbgp("[Query_Current_Task_Information]获取C_G2_town地图任务信息")
                            waypoint = api_GetTeleportationPoint()
                            if poe2_api.Waypoint_is_open("G3_town", waypoint) then
                                return bret.RUNNING
                            end
                            if player_info.current_map_name_utf8 ~= "C_G2_1" then
                                self.mas = "C_G2_1"
                            end
                            if poe2_api.get_team_info(team_info, config, player_info, 2) ~= "大號名" then
                                -- 发送任务信息
                        ...
                        elseif self.mas == "C_G1_town" then
                            poe2_api.dbgp("[Query_Current_Task_Information]获取C_G1_town地图任务信息")
                            waypoint = api_GetTeleportationPoint()
                            if poe2_api.Waypoint_is_open("C_G2_1", waypoint) then
                                return bret.RUNNING
                            end
                            if poe2_api.Waypoint_is_open("C_G1_town", waypoint) then
                                if poe2_api.get_team_info(team_info, config, player_info, 2) ~= "大號名" then
                                    -- 发送任务信息
                                    env.map_name = "C_G1_2"
                                    return bret.SUCCESS
                                ...
                                if player_info.current_map_name_utf8 == "G3_town" then
                                    env.interaction_object = { "崎點" }
                                    if poe2_api.get_team_info(team_info, config, player_info, 2) ~= "大號名" then
                                        -- 发送任务信息
                                        env.map_name = self.mas
                                        return bret.SUCCESS
                        ...
                        elseif self.mas == "C_G3_town" then
                            poe2_api.dbgp("[Query_Current_Task_Information]获取C_G3_town地图任务信息")
                            if poe2_api.get_team_info(team_info, config, player_info, 2) ~= "大號名" then
                                -- 发送任务信息
                                env.map_name = self.mas
                                return bret.SUCCESS
                            end
                        elseif self.mas == "C_G3_7" then
                            poe2_api.dbgp("[Query_Current_Task_Information]获取C_G3_7地图任务信息")
                            env.grid_x = nil
                            env.grid_y = nil
                            if poe2_api.table_contains(finished_tasks, "部落復仇") then
                                if poe2_api.get_team_info(team_info, config, player_info, 2) ~= "大號名" then
                                    if not next(self.raw) or (self.raw_time ~= 0 and api_GetTickCount64() - self.raw_time > 16 * 1000) then
                                        -- 发送任务信息
                                        self.raw_time = api_GetTickCount64()
    ...
    Is_Move = {
        run = function(self, env)
            ...
            function is_point(grid_x, grid_y)
            ...
            function away_monster(range_info, monster_id)
                -- 使用循环代替生成器表达式
                for _, i in ipairs(range_info) do
            ...
            if mate then
                ...
                if door_list and poe2_api.point_distance(door_list[1].grid_x, door_list[1].grid_y, player_info) < 15 then
                    if poe2_api.find_text({ UI_info = UI_info, text = "門", min_x = 0 }) and door_list[1].is_selectable
                        and not poe2_api.table_contains(player_info.current_map_name_utf8, { "G1_15", "G3_8", "G3_14", "C_G1_15", "C_G3_8", "C_G3_14" }) then
                        api_ClickMove(door_list[1].grid_x, door_list[1].grid_y, door_list[1].world_z, 1)
                ...
                if monster then
                    monster_distacne = poe2_api.get_point_distance(mate.grid_x, mate.grid_y, monster.grid_x,
                        monster.grid_y)
                    if (monster and poe2_api.table_contains(poe2_api.get_team_info(team_info, user_config, player_info, 2), { "大號名", "未知" })
                        ...
                        if monster_info then
                            if not away_monster_info then
                                ...
                                point_monster = api_FindNearestReachablePoint(away_monster_info.grid_x,
                                    away_monster_info.grid_y, 25, 0)
                                env.end_point = { point_monster.x, point_monster.y }
                            end
                            if poe2_api.point_distance(monster_info.grid_x, monster_info.grid_y, player_info) < 30 then
                                env.monster_info = nil
                            end
                        end
    ...
    Is_Attack = {
        run = function(self, env)
            ...
            local monster = is_monster(range_info_sorted)
            if poe2_api.table_contains(poe2_api.get_team_info(team_info, user_config, player_info, 2), { "大號名", "未知" }) then
                env.monster_info = nil
                return bret.SUCCESS
            end
            local distance = poe2_api.point_distance(mate.grid_x, mate.grid_y, player_info)
    ...
    Click = {
        run = function(self, env)
            ...
            poe2_api.while_click(UI_info, "復甦", mate, range_info, is_leader)
            return bret.RUNNING
        end
    },
    ...
    Is_Pick_Up_Task_Props = {
        run = function(self, env)
            ...
            local function is_items()
                if range_items then
                    for _, item in range_items do
                        if player_info.current_map_name_utf8 == "G1_1" and user_new_item[item.category] and item.grid_x ~= 0 then
                            local point = poe2_api.af_api.FindNearestReachablePoint(item.grid_x, item.grid_y, 20, 0)
                            if not api_findPath(player_info.grid_x, player_info.grid_y, point.x, point.y) then
                                return false
                            end
                            return true
                        elseif poe2_api.table_contains(item.category, { 'QuestItem', "Active Skill Gem" }) and not poe2_api.table_contains(item.baseType_utf8, { "黃金", "金幣", "紅色蘑菇", "綠色蘑菇", "藍色蘑菇", "龍蜥最後通牒雕刻" }) and item.grid_x ~= 0 then
                            local point = poe2_api.af_api.FindNearestReachablePoint(item.grid_x, item.grid_y, 20, 0)
                            if not api_findPath(player_info.grid_x, player_info.grid_y, point.x, point.y) then
                                return false
                            end
    ...
    Pick_Up_Ground_Item = {
        run = function(self, env)
            ...
            if poe2_api.find_text({ UI_info = UI_info, text = "背包" }) then
                poe2_api.click_keyboard("i")
                poe2_api.dbgp("开背包2")
            end
            if range_items then
                for _, item in range_items do
                    if item.baseType_utf8 == item_name then
                        if poe2_api.find_text({ UI_info = UI_info, text = item.baseType_utf8, sorted = true }) then
                            ...
                            api_ClickMove(poe2_api.toInt(item.grid_x), poe2_api.toInt(item.grid_y),
                                poe2_api.toInt(player_info.world_z), 1)
                        end
                        api_Sleep(100)
                    end
        ...
        run = function(self, env)
            ...
            if poe2_api.table_contains(current_map, { "G3_12", "C_G3_12", "G2_4_3", "C_G2_4_3", "G1_15", "C_G1_15" }) then
                local function check_role_path(names)
                    ...
                    for _, actor in ipairs(actors) do
                        if actor.name_utf8 == names then
                            ...
                            if api_findPath(point.x, point.y, target.x, target.y) then
                                return true
                            end
                        end
                    end
                ...
                if not check_role_path(team_member_3) and check_pos_dis(team_member_3) and check_pos_dis(team_member_3) > 30 then
                    ...
                    if api_GetTickCount64() - self.time > 30 * 1000 then
            ...
            if poe2_api.table_contains(current_map, { "G3_1", "C_G3_1" }) and poe2_api.table_contains(task_area, { "G3_1", "C_G3_1" }) then
                poe2_api.dbgp("检测到任务区域:G3_1")
                return bret.SUCCESS
            end
            if poe2_api.table_contains(current_map, { "G3_2_2", "C_G3_2_2" }) and poe2_api.table_contains(task_area, { "G3_2_2", "C_G3_2_2" }) and
                poe2_api.table_contains(party_pos(team_member_3), { "G3_2_2", "C_G3_2_2" }) then
                poe2_api.dbgp("检测到任务区域:G3_2_2")
                return bret.RUNNING
            end
            if current_map == "G3_town" and poe2_api.table_contains(task_area, { "G3_14", "G3_16", "G3_17", "C_G3_14", "C_G3_16_", "C_G3_17" }) or poe2_api.table_contains(task_name, { "回到過去，進入奧札爾", "穿過城鎮中的時空傳送門以移至殘忍難度" }) then
                ...
                if not waypoint_pos and not point then
                    ...
                    if a and b then
                        if a < 120 and b < 50 then
                            ...
                            if poe2_api.table_contains(player_info.current_map_name_utf8, "town") then
                                local towards_point = poe2_api.move_towards({ player_info.grid_x, player_info.grid_y },
                                    waypoint_pos, 20)
                                api_ClickMove(poe2_api.toInt(towards_point[1]), poe2_api.toInt(towards_point[2]),
                                    poe2_api.toInt(player_info.world_z), 0)
                                api_Sleep(100)
                                poe2_api.click_keyboard("space")
                            end
                        end
            ...
            if poe2_api.table_contains(task_area, { "G3_14", "G3_16", "G3_17", "C_G3_14", "C_G3_16_", "C_G3_17", "G3_12", "C_G3_12" }) and not poe2_api.table_contains(party_pos(team_member_3), { "G3_town", "G3_14", "G3_16", "G3_17", "C_G3_town", "C_G3_14", "C_G3_16_", "C_G3_17", "G3_12", "C_G3_12" }) then
            ...
            if string.find(current_map, "G3_12") and ((interaction_object and string.find(interaction_object, "召瓦尔") and check_pos_dis(team_member_3)) or (check_pos_dis("艾瓦") and check_pos_dis("艾瓦") < 25) or not check_pos_dis("競技場")) then
                return bret.RUNNING
            end
            local count = 3
            if poe2_api.find_text({ UI_info = UI_info, text = poe2_api.task_area_list_data(party_pos(team_member_3))[1][2], min_x = 0, min_y = 0, max_x = 195, max_y = 590 }) then
                for i = 0, count - 1 do
                    if not poe2_api.find_text({ UI_info = UI_info, text = "你確定要傳送至此玩家的位置？" }) then
            ...
            if check_pos_dis(team_member_3) and check_pos_dis(team_member_3) > 100 and poe2_api.table_contains(current_map, { "G1_15", "G2_3", "C_G1_15", "C_G2_3" }) then
        ...
        run = function(self, env)
            ...
            local function party_path_memember()
                ...
                for _, actor in ipairs(actors) do
                    ...
                    if api_findPath(player_info.grid_x, player_info.grid_y, point.x, point.y) then
                        match_count = match_count + 1
            ...
            local function check_role_path(names)
                ...
                for _, actor in ipairs(range_info) do
                    ...
                    if actor.name_utf8 == names and api_findPath(near_point.x, near_point.y, near_point_actor.x, near_point_actor.y) then
                        return true
                    end
                end
            ...
            local function check_pos_dis(names)
                poe2_api.dbgp("[check_pos_dis]获取位置距离:")
            ...
            if not self.follow and ((special_map_point and interaction_object and string.find(me_area, "G2_town")) or (me_area ~= task_area and poe2_api.table_contains(task_area, { "G3_1", "C_G3_1", "G3_2_2", 'C_G3_2_2' }) and poe2_api.table_contains(party_pos(team_member_4), { "G3_1", "C_G3_1", "G3_2_2", 'C_G3_2_2' }))
                    or (me_area ~= task_area and poe2_api.table_contains(task_area, { "G3_12", "C_G3_12", "G3_14", "G3_16", "G3_17", "C_G3_14", "C_G3_16_", "C_G3_17", "C_G1_1" })) or poe2_api.table_contains(task_name, { "回到過去，進入奧札爾", "探索科佩克神殿並尋找瓦爾的知識展覽室" })) then
                local point = get_range_pos("倉庫")
                local waypoint_point = get_range_pos("傳送點")
                local waypoint_pos = get_range_pos("崎點")
                local distance_between = nil
                ...
                local count = 3
                if not check_pos_dis(team_member_4) then
                    for i = 1, count do
                        if not poe2_api.find_text({ UI_info = UI_info, text = "你確定要傳送至此玩家的位置？" }) then
                            local x, y = poe2_api.get_member_name_according(UI_info, team_member_4)
                            if y ~= 0 then
            ...
            if task_area == "C_G1_1" and not poe2_api.table_contains(me_area, { "C_G1_1", "G3_town" }) then
                env.map_name = G3_town
            end
            if poe2_api.table_contains(me_area, { "G1_15", "C_G1_15" }) and poe2_api.table_contains(task_area, { "G1_15", "C_G1_15" }) then
                if poe2_api.is_have_mos({ range_info = range_info, player_info = player_info }) then
                    return bret.SUCCESS
                end
                if next_level() and not check_pos_dis(boss_name) and party_dis_memember(range_info) and party_path_memember() then
                    self.follow = true
            ...
            if poe2_api.table_contains(me_area, { "G2_4_3", "C_G2_4_3" }) and poe2_api.table_contains(task_area, { "G2_4_3", "C_G2_4_3" }) then
                if poe2_api.is_have_mos({ range_info = range_info, player_info = player_info }) then
                    return bret.SUCCESS
                end
                if poe2_api.find_text({ UI_info = UI_info, text = "掩埋神殿", min_x = 0 }) and not party_path_memember() then
                    if not poe2_api.click_text_UI({ text = "respawn_at_checkpoint_button", UI_info = UI_info }) and not self.bool1 then
                        local walkpoint = api_FindRandomWalkablePosition(player_info.grid_x, player_info.grid_y, 50)
                        api_ClickMove(poe2_api.toInt(walkpoint.x), poe2_api.toInt(walkpoint.y),
                            poe2_api.toInt(player_info.world_z), 7)
            ...
            if poe2_api.table_contains(me_area, { "G3_12", "C_G3_12" }) and poe2_api.table_contains(task_area, { "G3_12", "C_G3_12" }) and not poe2_api.is_have_boss_distance(range_info, player_info, boss_name) then
                if poe2_api.is_have_mos({ range_info = range_info, player_info = player_info }) then
                    return bret.SUCCESS
                end
                if (next_level() or not check_role_path(team_member_4)) and check_pos_dis(team_member_4) and check_pos_dis(team_member_4) > 30 and not check_role_path(team_member_4) then
                    self.follow = true
            ...
            if string.find(me_area, "G3_town") then
                if not check_pos_dis("傳送點") then
                    if not check_pos_dis("崎點") and poe2_api.find_text({ UI_info = UI_info, text = "淹沒之城", min_x = 0 })
                        ...
                        return bret.RUNNING
                    elseif interaction_object and poe2_api.table_contains(interaction_object, "科佩克") and poe2_api.find_text({ UI_info = UI_info, text = "淹沒之城", min_x = 0 }) then
                        poe2_api.find_text({ UI_info = UI_info, text = "淹沒之城", min_x = 0, click = 2 })
                        self.click_ke = true
                        return bret.RUNNING
                    end
            ...
            if me_area == task_area then
                if not string.find(me_area, "own") then
                    if (not party_pos_memember(me_area) or not party_dis_memember(range_info)) and not self.back_city then
                        ...
                        local retime = api_GetTickCount64() - self.time
                ...
                if arena_list and arena_list[1].hasLineOfSight and arena_list[1].is_selectable and api_findPath(player_info.grid_x, player_info.grid_y, arena_list[1].grid_x, arena_list[1].grid_y) then
                    if poe2_api.is_have_mos({ range_info = range_info, player_info = player_info }) then
                        return bret.SUCCESS
                    end
                ...
                if poe2_api.table_contains(me_area, { "G1_15", "G2_3", "G3_12", "C_G1_15", "C_G2_3", "C_G3_12" }) then
                    local louti = get_range_pos("樓梯")
                    if poe2_api.table_contains(me_area, { "G1_15", "C_G1_15" }) then
                        if not mini_map_obj("GargoyleInactive") and mini_map_obj("Waypoint") then
                            return bret.SUCCESS
                        end
                    end
                    ...
                    elseif not next_level() and louti then
                        ...
                        if louti and (not poe2_api.table_contains(me_area, { "G2_3", "C_G2_3" }) or not poe2_api.find_text({ UI_info = UI_info, text = "樓梯", min_x = 0 })) then
                            if check_pos_dis("樓梯") > 50 then
    ...
    Is_Open_Task_Wayoint = {
        run = function(self, env)
            ...
            if poe2_api.task_area_list_data(task_area)[2] == "有" and poe2_api.Waypoint_is_open(task_area, waypoint) then
                if string.find(task_area, "C_G2") and not string.find(current_map, "C_G2") then
                    env.teleport_area = C_G2_1
                    return py_trees.common.Status.SUCCESS
                elseif string.find(task_area, "G2") and not string.find(current_map, "G2") then
                    env.teleport_area = G2_1
                    return bret.SUCCESS
                end

                if string.find(task_area, "C_G3") and not string.find(current_map, "C_G3") then
                    env.teleport_area = C_G3_town
                    return bret.SUCCESS
                elseif string.find(task_area, "G3") and not string.find(current_map, "G3") then
                ...
                if task_area == "G3_12" or task_area == "C_G1_1" or task_area == "C_G3_12" then
                    env.teleport_area = task_area
                    return bret.SUCCESS
                end

                if not Waypoint_is_open(task_area, waypoint_open) and task_area ~= "G3_2_2" and task_area ~= "C_G3_2_2" then
                    env.waypoint = api_GetTeleportationPoints()
    ...
    Teleport_Is_Near = {
        run = function(self, env)
            ...
            if string.find(task_area, "^C_G1_1$") then
                poe2_api.dbgp("[task_area]C_G1_1")
                local waypoint_pos = get_actor_pos("崎點")
                if string.find(current_map, "G3_town") then
                    if not waypoint_pos and not poe2_api.find_text({ UI_info = UI_info, text = "崎點", click = 0 }) then
                        local target_pos = { 420, 836 }
                        if api_findPath(player_info.grid_x, player_info.grid_y, target_pos[1], target_pos[2]) then
                            env.end_point = { target_pos[1], target_pos[2] }
                            return bret.SUCCESS
                        end
                    else
            ...
            if (string.find(teleport_area, "G2_town") and not string.find(current_map, "G1_town")) or not check_pos_dis("傳送點") then
                ...
                elseif string.find(current_map, "G2_1$") and check_pos_dis("札卡") then
                    poe2_api.dbgp("[teleport_area]G2_1阿杜拉車隊")
                ...
                local waypoint_pos = get_actor_pos("崎點")
                if point then
                    if current_map == "G3_town" and waypoint_pos and poe2_api.table_contains(task_area, { "G3_14", "G3_16", "G3_17", "C_G3_14", "C_G3_16_", "C_G3_17" }) then
    ...
    Click_Map_To_Area_Teleport = {
        run = function(self, env)
            ...
            local teleport_area = env.teleport_area
            if string.find(teleport_area, "C_") then
                twice_map = false
            else
                twice_map = true
            end
            ...
            if #poe2_api.task_area_list_data(teleport_area) < 4 then
                map_pos = { { poe2_api.task_area_list_data(teleport_area)[3][1], poe2_api.task_area_list_data(teleport_area)[3][2] } }
            else
                map_pos = { { poe2_api.task_area_list_data(teleport_area)[3][1], poe2_api.task_area_list_data(teleport_area)[3][2] }, { poe2_api.task_area_list_data(teleport_area)[4][1], poe2_api.task_area_list_data(teleport_area)[4][2] } }
            end
            if current_map ~= teleport_area then
                if poe2_api.find_text({ UI_info = UI_info, text = "背包", click = 0, refresh = true }) then
                    poe2_api.click_keyboard("i")
                end
                if poe2_api.find_text({ UI_info = UI_info, text = "傳送點", click = 0, refresh = true }) then
                    ...
                    self.s_time = 0
                    if string.find(teleport_area, "G1") and twice_map then
                        ...
                        api_Sleep(200)
                    elseif string.find(teleport_area, "G2") and twice_map then
                        api_ClickScreen(1567, poe2_api.toInt(mini_top) + 22, 0)
                        api_Sleep(200)
                        api_ClickScreen(1567, poe2_api.toInt(mini_top) + 22, 1)
                        poe2_api.find_text({ UI_info = UI_info, text = "第 2 章", click = 2, refresh = true })
                        api_Sleep(200)
                    elseif string.find(teleport_area, "G3") and twice_map then
                        api_ClickScreen(1567, poe2_api.toInt(mini_top) + 22, 0)
                        api_Sleep(200)
                        api_ClickScreen(1567, poe2_api.toInt(mini_top) + 22, 1)
                    ...
                    local teleport_text = { "奧格姆郡，約恆曆", "七大水域之地", "奧札爾區域草稿 #", "古奧札爾草稿" }
                    if poe2_api.find_text({ UI_info = UI_info, text = teleport_text, click = 2, refresh = true, match = 2 }) then
                        if not poe2_api.find_text({ UI_info = UI_info, text = task_area_name, click = 0, refresh = true }) then
                            api_ClickScreen(poe2_api.toInt(map_pos[1][1]), poe2_api.toInt(map_pos[1][2]), 0)
                            if not string.find(teleport_area, "G3_town") and not poe2_api.find_text({ UI_info = UI_info, text = task_area_name, click = 0, refresh = true }) then
                                api_Sleep(300)
                                poe2_api.click_keyboard("space")
                                return bret.RUNNING
                            end
                            api_Sleep(300)
                            if #(poe2_api.task_area_list_data(teleport_area)) < 4 then
                                if not string.find(teleport_area, "own") and not string.find(teleport_area, "G1_7") and
                                    not poe2_api.find_text({ UI_info = UI_info, text = "副本管理員", click = 0, refresh = true }) then
                                    poe2_api.click_keyboard("ctrl", 1)
                                    api_Sleep(1000)
                                    api_ClickScreen(map_pos[1][1], map_pos[1][2], 1)
                                    api_Sleep(1000)
                                ...
                                if point then
                                    api_ClickScreen(int(point[1]), int(point[2]), 1)
                                else
                                    api_ClickScreen(int(map_pos[1][1]), int(map_pos[1][2]), 1)
                                end
                                return bret.RUNNING
                            end
                            api_ClickScreen(int(map_pos[1][1]), int(map_pos[1][2]), 1)
                    ...
                    if #(poe2_api.task_area_list_data(teleport_area)) > 3 then
                        api_Sleep(200)
                        api_ClickScreen(int(map_pos[2][1]), int(map_pos[2][2]), 0)
                        api_Sleep(200)
                        local task_two_area_name = poe2_api.task_area_list_data(teleport_area)[5]
                        ...
                        if not string.find(teleport_area, "own") and not string.find(teleport_area, "G1_7") and
                            ...
                            poe2_api.click_keyboard("ctrl", 1)
                            api_Sleep(1000)
                            api_ClickScreen(map_pos[2][1], map_pos[2][2], 1)
                            api_Sleep(1000)
                            poe2_api.click_keyboard("ctrl", 2)
                        end
                        local point = poe2_api.find_text({ UI_info = UI_info, text = "新副本", min_x = 0, position = 3, refresh = true })
    ...
    Not_In_New_Area_Skill = {
        run = function(self, env)
            ...
            if player_info.current_map_name_utf8 == "G1_1" or (poe2_api.find_text({ UI_info = UI_info, text = "抵達皆伐" }) and not string.find(player_info.current_map_name_utf8, "C_")) then
                poe2_api.print_log('全部技能信息')
                local allskill_info = api_GetAllSkill()
                if allskill_info then
                    env.allskill_info = allskill_info
                else
                    return bret.RUNNING
                end
                poe2_api.print_log('技能宝石列表信息')
                env.skill_gem_info = api_Getinventorys(0x2f)
                return bret.FAILURE
            end
            if poe2_api.find_text({ UI_info = UI_info, text = "技能", min_x = 0, min_y = 32, max_x = 381, max_y = 81 }) then
                poe2_api.click_keyboard("esc")
                return bret.RUNNING
            end
            return bret.SUCCESS
    ...
    Need_Attack_Check = {
        run = function(self, env)
            ...
            local team_info = env.team_info
            local user_info = env.user_info
            local attack_dis_map = 100
            if (poe2_api.is_have_boss_distance({range_info = range_info, player_info = player_info, boss_name = boss_name, 180}) 
                ...
                and poe2_api.get_team_info(team_info, user_info, player_info, 2) == "大號名" then
                return bret.FAIL        
            else
                return bret.SUCCESS        
            end
    ...
    ReleaseSkillAction = {
        run = function(self, env)
            ...
            for _, monster in ipairs(env.range_info) do
                ...
                if poe2_api.table_contains(monster.type , {1,3}) or                  -- 类型检查
                ...
                if (boss_name and string.find(monster.name_utf8, boss_name)) or monster.rarity == 3 then
            ...
            if valid_monsters then
                ...
                if not (#env.center_point > 0 and env.center_radius > 0) then
                    ...
                    for _, monster in ipairs(env.range_info) do
                        ...
                        -- 快速失败条件检查
                        if poe2_api.table_contains(monster.type , {1,3}) or 
            ...
            if valid_monsters then
                ...
                if valid_monsters.name_utf8 == "撕裂者" then
                    for _,v in ipairs(current_map_info) do
                        if v.name_utf8 == "RathbreakerActive" and math.sqrt((k.grid_x - player_info.grid_x) * 2 + (k.grid_y - player_info.grid_y) * 2)>50 then
                ...
                if available_skills and #available_skills > 0 then
                    ...
                    if distance > 25 then
                        ...
                        for _, prop in ipairs(my_game_info.first_magicProperties or {}) do
                            if not poe2_api.table_contains(valid_monsters.magicProperties, prop) then
                                goto continue_prop
                            end
                            ::continue_prop::
                        end
        ...
        run = function(self, env)
            ...
            local _handle_space_action = function(monster, space_flag, space_monsters, space_time, player_info)
                ...
                api_GetTickCount64() - self.last_space_time >= space_time then
                    ...
                    api_Sleep(200)
                    poe2_api.dbgp1("gvdsfhgbreshbdfb")
                    poe2_api.click_keyboard("space")
            ...
            if space then
                for _, monster in ipairs(monsters) do
                    ...
                    if poe2_api.table_contains(monster.name_utf8, {'多里亞尼','崛起之王．賈嫚拉'}) and monster.life > 0 and dis and dis < 20 then
                        _handle_space_action(monster, space, space_monster, space_time, player_info)
                    end
                    if monster.life > 0 and monster.isActive and poe2_api.table_contains(monster.name_utf8, {"","惡魔"}) and monster.type == 1 and dis and dis < 20 and 
            ...
            if _handle_space_action_path_name( player_info) then
               env.end_point = nil
               env.path_list = {}
            end
            return bret.SUCCESS
            ...
            if type(interactive_object) == "string" then
                ...
                if target_obj.name_utf8 == "MapDevice" then
                    ...
                    if maps then
                        ...
                        api_ClickMove(poe2_api.toInt(maps.grid_x), poe2_api.toInt(maps.grid_y),
                            poe2_api.toInt(maps.world_z - 70), 0)
                        api_Sleep(800)
                    end
                ...
                if not poe2_api.find_text({ UI_info = env.UI_info, text = interactive_object, click = 2, max_x = 1200, refresh = true }) then
                    api_ClickMove(poe2_api.toInt(target_obj.grid_x), poe2_api.toInt(target_obj.grid_y),
                        poe2_api.toInt(player_info.world_z), 1)
                end
                api_Sleep(100)
            else
                ...
                if interactive_object and (not text or text == "" or poe2_api.table_contains(text, { "門", "聖潔神殿" }) or not poe2_api.find_text({ text = text, UI_info = env.UI_info, min_x = 200, max_y = 750, match = 2, max_x = 1200, sorted = true, click = 2 })) then
                    if poe2_api.table_contains(text, { "門", "聖潔神殿" }) then
                        ...
                        api_ClickMove(poe2_api.toInt(interactive_object.grid_x),
                            poe2_api.toInt(interactive_object.grid_y), poe2_api.toInt(player_info.world_z - 70), 1)
                    else
                        if env.need_item and env.need_item == interactive_object then
                            api_ClickMove(poe2_api.toInt(interactive_object.grid_x),
                                poe2_api.toInt(interactive_object.grid_y), poe2_api.toInt(player_info.world_z), 1)
                        else
                            local ok, value = pcall(function()
                            ...
                            if ok and value ~= nil then
                                if string.find("Metadata/Terrain/Leagues/Ritual/RitualRuneInteractable", interactive_object.path_name_utf8) then
                                    ...
                                    api_ClickMove(poe2_api.toInt(interactive_object.grid_x),
                                        poe2_api.toInt(interactive_object.grid_y),
                                        poe2_api.toInt(player_info.world_z - 250), 1)
                                end
                            else
                                api_ClickMove(poe2_api.toInt(interactive_object.grid_x),
                                    poe2_api.toInt(interactive_object.grid_y), poe2_api.toInt(player_info.world_z - 250),
                                    1)
                            end
                        end
                    end
    ...
    GET_Path = {
        run = function(self, env)
            ...
            poe2_api.time_p("计算路径成功 耗时 -->", api_GetTickCount64() - start_current_time,
                "两点：{" .. player_position.x .. "," .. player_position.y .. "} -> {" .. point.x .. "," .. point.y .. "}")
            ...
            if result and #result > 0 then
                ...
                if poe2_api.table_contains(player_info.current_map_name_utf8, "own") then
                    ...
                    api_ClickMove(poe2_api.toInt(result.x), poe2_api.toInt(result.y), poe2_api.toInt(player_info.world_z),
                        0)
                    poe2_api.click_keyboard("space")
                end
                return bret.RUNNING
        ...
        run = function(self, env)
            ...
            if roll_time > 5 * 1000 and distance < self.movement_threshold then
                ...
                if point then
                    ...
                    if player_point then
                        api_ClickMove(poe2_api.toInt(player_point.x), poe2_api.toInt(player_point.y),
                            poe2_api.toInt(player_info.world_z - 70), 7)
                    end
                end
                poe2_api.click_keyboard("space")
                env.roll_time = nil
            ...
            if current_time - self.last_move_time >= move_interval * 1000 then
                if point then
                    ...
                    api_ClickMove(poe2_api.toInt(point[1]), poe2_api.toInt(point[2]), poe2_api.toInt(player_info.world_z),
                        7)
