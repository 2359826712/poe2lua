...
local custom_nodes = {
    ...
    Not_Main_Page = {
        run = function(self, env)
            ...
            -- 大号，小号更新障碍
            if poe2_api.get_team_info(env.team_info, env.user_config, player_info, 2) == "大號名" then
                if poe2_api.table_contains(player_info.current_map_name_utf8, { "G2_3", "G2_9_1","G3_17" }) and not poe2_api.find_text({ UI_info = env.UI_info, text = "競技場", min_x = 0 }) then
    ...
    Team = {
        run = function(self, env)
            ...
            if team_info_data ~= nil then
                ...
                if is_subset then
                    if poe2_api.find_text({UI_info = nil, text = "社交", min_x = 0, min_y = 32, max_x = 381, max_y = 81}) then
                        poe2_api.click_keyboard("j")
                        api_Sleep(500)
                        return bret.RUNNING
                    end
    ...
    Check_Role = {
        run = function(self, env)
            ...
            local function parse_data_string(data_str)
                ...
                -- 使用正则表达式匹配键值对
                for key, value in data_str:gmatch("'([^']+)':%s*([^,]+)") do
                    result[key] = value:gsub("^%s*(.-)%s*$", "%1")  -- 去除前后空格
                end
            ...
            local function direct_parse_log_line(line, max_age_ms)
                -- 直接按位置提取
                -- 2025/09/12 17:26:15 1129408828 3ef232c2 [INFO Client 26868] %king_qq: 'user_name': player_info.name_utf8, 'map_name': task.map_name, 'task_name': task.task_name, 'task_index': task.index,
                ...
                local current_timestamp = api_GetTickCount64()
            ...
            local function process_recent_logs_unique(file_path, max_age_ms)
                ...
                for _, line in ipairs(reversed_lines) do
                    ...
                    if parsed_line then
                        ...
                        if not seen_names[name] or timestamp > seen_names[name] then
                            ...
                            latest_data[name] = data  -- 直接保存解析后的数据
                        end
                    else
                        -- 如果遇到超时的记录，提前停止
                        break
                ...
                return latest_data  -- 返回格式：{name = parsed_data}
            end
            -- 获取队伍成员列表（排除自己）
            local function get_team_members()
                local members = {}
                for _, member in ipairs(team_info) do
                    if member and member.name_utf8 and member.name_utf8 ~= player_info.name_utf8 then
                        table.insert(members, member.name_utf8)
                    end
                end
                return members
            end
            ...
            -- 统计有效任务数量
            for name, mission_data in ipairs(member_task_info) do
    ...
    Query_Current_Task_Information = {
        run = function(self, env)
            ...
            if task and next(task) then
                poe2_api.dbgp("检测到任务：", poe2_api.printTable(task))
                self.mas = nil
                ...
                if poe2_api.get_team_info(team_info, config, player_info, 1) ~= "大號名" then
                    ...
                    if not poe2_api.table_contains(player_info.current_map_name_utf8, { "G1_1" }) and not poe2_api.find_text({ UI_info = env.UI_info, text = "抵達皆伐" }) then
                        if not next(self.raw) or (self.raw_time ~= 0 and api_GetTickCount64() - self.raw_time > max_time) then
                            -- 发送任务信息
                            local task_text = "task_name=" .. task.task_name .. ",task_index=" .. task.index ..",map_name=" .. task.map_name
                            poe2_api.click_keyboard("enter")
                            api_Sleep(500)
                            poe2_api.paste_text("%"..task_text)
                            api_Sleep(500)
                            poe2_api.click_keyboard("enter")
                            api_Sleep(500)
            ...
            if poe2_api.find_text({ UI_info = UI_info, text = poe2_api.task_area_list_data(party_pos(team_member_3))[1][2], min_x = 0, min_y = 0, max_x = 195, max_y = 590 }) then
                for i = 0, count - 1 do
                    if not poe2_api.find_text({ UI_info = UI_info, text = "你確定要傳送至此玩家的位置？" }) then
                        ...
                        if y ~= 0 then
                            local rand_x = 14 + math.random(-7, 7)
                            local rand_y = y  + math.random(-7, 7)
                            api_ClickScreen(poe2_api.toInt(rand_x), poe2_api.toInt(rand_y), 0)
                            api_Sleep(500)
                            api_ClickScreen(poe2_api.toInt(rand_x), poe2_api.toInt(rand_y), 1)
                        end
    ...
    DodgeAction = {
        ...
        run = function(self, env)
            ...
            local _handle_space_action_path_name1 = function(player_info, space_time)
                ...
                if player_info.isInDangerArea then
                    local ret = api_GetSafeAreaLocation(player_info.grid_x, player_info.grid_y, 60, 10, 0, 0.5)
            ...
            if space then
                for _, monster in ipairs(monsters) do
                    ...
                    if monster.life > 0 and monster.isActive and poe2_api.table_contains(monster.name_utf8, {"","惡魔"}) and dis and dis < 20 and 
    ...
    Is_Have_Interaction_Object = {
        run = function(self, env)
            ...
            local user_config = env.user_config
            local team_member_2 = poe2_api.get_team_info(team_info,user_config,player_info,2)
            if me_area == "G2_1" and interaction_object and team_member_2 == "大號名" then
                poe2_api.dbgp("大号在G2_1,不需要交互")
                return bret.SUCCESS
            end
            ...
            if interaction_object and me_area == task_area then
        ...
        run = function(self, env)
            ...
            if interaction_object or interaction_object_map_name then
                            ...
                            if door.is_selectable then
                                ...
                                if get_distance(door.grid_x,door.grid_y) <20 then
                                    ...
                                    if api_FindPath(local_x,local_y,door_point.x,door_point.y) then
                                        ...
                                        api_ClickMove(poe2_api.toInt(door_point.x),poe2_api.toInt(door_point.y),poe2_api.toInt(door.world_z),0)
                                        poe2_api.click_keyboard("space")
                ...
                if poe2_api.table_contains("壓桿",interaction_object_set) then
                    ...
                    local target = mini_map_obj("WaterwaysLever")
                    ...
                    if target and #target > 0 then
                        ...
                        if get_distance(target[1].grid_x, target[1].grid_y) <25 then
                            ...
                            if api_FindPath(local_x,local_y,target_point.x,target_point.y) then
                                ...
                                api_ClickMove(poe2_api.toInt(target_point.x),poe2_api.toInt(target_point.y),poe2_api.toInt(target[1].world_z),0)
                                poe2_api.click_keyboard("space")
                                env.not_need_active = true
                                return bret.RUNNING
                            end
                ...
                if poe2_api.table_contains("<questitem>{發電機}",interaction_object_set) then
                    ...
                    if (not bag_info or #bag_info == 0) or ((not poe2_api.check_item_in_inventory("中型靈魂核心",bag_info)) or poe2_api.find_text({ UI_info = UI_info, text = target, max_x=1347, min_x=200}))
                        ...
                        if poe2_api.find_text({ UI_info = UI_info, text = target, max_x=1347, min_x=200}) then
                ...
                if poe2_api.table_contains("召喚瑟維",interaction_object_set) then
                    ...
                    if servi and #servi > 0 then
                        ...
                        api_ClickMove(poe2_api.toInt(sister_toward[1]),poe2_api.toInt(sister_toward[2]),poe2_api.toInt(player_info.world_z),0)
                        poe2_api.click_keyboard("space")
                        api_Sleep(300)
                    ...
                    if handle and #handle > 0 and player_info.current_map_name_utf8 == "G1_13_2" then
                        for _,h in ipairs(handle) do
                            if h.stateMachineList and h.stateMachineList.helper_state ~= 2 then
                                if get_distance(h.grid_x,h.grid_y) < 40 then
                                    ...
                                    api_ClickMove(poe2_api.toInt(handle_point.x),poe2_api.toInt(handle_point.y),poe2_api.toInt(h.world_z),0)
                                    api_Sleep(300)
                                    poe2_api.click_keyboard("space")
                                    env.not_need_active = true
                                    return bret.RUNNING
                ...
                if player_info.current_map_name_utf8 == "G2_1" then
                    if not mini_map_obj("CroneInactive") then
                ...
                elseif player_info.current_map_name_utf8 == "G2_4_1" then
                    if not mini_map_obj("KabalaInactive") then
                        interaction_object_set = nil 
                        env.interaction_object = nil
                ...
                elseif player_info.current_map_name_utf8 == "G3_3" then
                    if not mini_map_obj("SilverbackBlackfistBossInactive") then
                        interaction_object_set = nil 
                        env.interaction_object = nil
                ...
                elseif player_info.current_map_name_utf8 == "G3_6_1" and not poe2_api.table_contains("艾瓦",interaction_object_set) then
                    if not mini_map_obj("BlackjawBossInactive") then
                            ...
                            if obj.name_utf8 == "符文之印" then
                                if obj.stateMachineList and obj.stateMachineList.ready ==1 and obj.stateMachineList.stabbed == 0 then
                                    if get_distance(obj.grid_x,obj.grid_y) < 25 then
                                        ...
                                        if api_GetTickCount64() - self.time > 6*1000 then
                                            local obj_walk_point = api_FindRandomWalkablePosition(obj.grid_x, obj.grid_y,30)
                                            api_ClickMove(obj_walk_point.x,obj_walk_point.y,obj.world_z,0)
                                            poe2_api.click_keyboard("space")
                                            self.time = 0
                                        end
                                        return bret.RUNNING
                        ...
                        if get_distance(record_map[1],record_map[2]) < map_distance then
                            ...
                            if boss_name and #boss_name > 0 then
                                ...
                                if boss_list and #boss_list > 0 and boss_list[1].life >0 then
                                    ...
                                    api_ClickMove(poe2_api.toInt(toward_boss.x),poe2_api.toInt(toward_boss.y),poe2_api.toInt(boss.world_z),1)
                                    api_Sleep(100)
                                    poe2_api.click_keyboard("spcae")
                                end
                                local toward_record_map = poe2_api.move_towards({local_x,local_y},{record_map[1],record_map[2]},20)
                                api_ClickMove(poe2_api.toInt(toward_record_map.x),poe2_api.toInt(toward_record_map.y),poe2_api.toInt(player_info.world_z),1)
                                api_Sleep(100)
                                poe2_api.click_keyboard("spcae")
                            end
                            env.is_arrive_end = true
    ...
    Move_To_Near_Leader_Point = {
        run = function(self, env)
            ...
            if api_GetTickCount64() - self.current_time > self.timeout then
                ...
                api_ClickMove(poe2_api.toInt(safe_point.x), poe2_api.toInt(safe_point.y) ,poe2_api.toInt(player_info.world_z), 0)
                api_Sleep(200)
                poe2_api.click_keyboard("spcae")
                self.current_time = 0
                return bret.RUNNING
            ...
            if poe2_api.find_text({UI_info = env.UI_info,text = "Checkpoints",min_x=0,max_y=30}) then
                poe2_api.click_keyboard("esc")
            end
            ...
            if target and #target > 0 then
                if poe2_api.point_distance(target.grid_x, target.grid_y, player_info) <= 10 then
                    ...
                    if dis >= 20 then
                        api_ClickMove(poe2_api.toInt(player_walk_point.x), poe2_api.toInt(player_walk_point.y) ,poe2_api.toInt(player_info.world_z), 0)
                        api_Sleep(200)
                        poe2_api.click_keyboard("spcae")
                        return bret.RUNNING
                    end
                ...
                elseif poe2_api.point_distance(target.grid_x, target.grid_y, player_info) <= 70 then
                    self.bool = false
                    self.current_time = 0
                    api_ClickMove(poe2_api.toInt(target.x), poe2_api.toInt(target.y) ,poe2_api.toInt(player_info.world_z), 0)
                    api_Sleep(200)
                    api_ClickMove(poe2_api.toInt(target.x), poe2_api.toInt(target.y) ,poe2_api.toInt(player_info.world_z), 7)
                    api_Sleep(200)
                    return bret.RUNNING
                end
            end
    ...
    GET_Path_Follow = {
        run = function(self, env)
            ...
            if result and #result > 0 then
                ...
                local arena_list =poe2_api.get_sorted_obj("競技場",range_info,player_info)
                if arena_list and arena_list[1].hasLineOfSight and arena_list[1].is_selectable then
                ...
                elseif arena_list and not arena_list[1].hasLineOfSight then
                    api_RestoreOriginalMap()
                else
                    local player_walk_point = api_FindRandomWalkablePosition(player_info.grid_x, player_info.grid_y, 30)
                    api_ClickMove(poe2_api.toInt(player_walk_point.x), poe2_api.toInt(player_walk_point.y) ,poe2_api.toInt(player_info.world_z), 7)
                end
                api_ClickMove(poe2_api.toInt(reachable_point.x), poe2_api.toInt(reachable_point.y) ,poe2_api.toInt(player_info.world_z), 7)
                api_RestoreOriginalMap()
                return bret.RUNNING
            end
            return bret.SUCCESS
    ...
    Move_To_Far_Leader_Point = {
        run = function(self, env)
            ...
            if env.roll_time > self.timeout and distance < self.movement_threshold and player_info.life > 0 then
                ...
                api_ClickMove(poe2_api.toInt(leader_point[1]), poe2_api.toInt(leader_point[2]), poe2_api.toInt(player_info.world_z), 0)
                api_Sleep(500)
                poe2_api.click_keyboard("space")
                env.roll_time = nil
                env.path_list_follow = {}
            ...
            if env.exit_time > 60*1000  and distance < self.movement_threshold and player_info.life > 0 then
                ...
                env.exit_time = nil
                if not poe2_api.table_contains(self.special_maps, player_info.current_map_name_utf8) and player_info.life >0 and 
                    not poe2_api.is_have_mos({range_info = range_info, player_info = player_info}) and not player_info.isInBossBattle then
                    env.is_timeout_exit = True
                    return bret.RUNNING
                end
            elseif distance >= self.movement_threshold or player_info.life == 0 then
            ...
            if point then
                ...
                if poe2_api.point_distance(reach_point.x,reach_point.y,player_info) < 25 then
                    if env.path_list_follow and #env.path_list_follow > 1 then
            ...
            if point.x == -1 and point.y == -1 then
                ...
                if poe2_api.table_contains(current_map, special_maps_3) then
                    ...
                    if walk_point and #walk_point > 0 and walk_point.x == -1 then
                        api_ClickMove(poe2_api.toInt(walk_point.x), poe2_api.toInt(walk_point.y),poe2_api.toInt(player_info.world_z),7)
                    end 
                ...
                if poe2_api.table_contains(current_map, special_maps_2) then
                    ...
                    if rand_pos then
                        ...
                        -- 点击移动到随机位置
                        api_ClickMove(poe2_api.toInt(rand_pos.x), poe2_api.toInt(rand_pos.y), poe2_api.toInt(player_info.world_z), 7)
    ...
    Is_Path_Move = {
        run = function(self, env)
            ...
            if env.is_arrive_end == false then
                return bret.FAIL
            end
        end
    },
